...
public class RedisUtil  {
	...
	private final static JdkSerializationRedisSerializer redisSerializer=new JdkSerializationRedisSerializer();
	// /**
	...
	// * @param timeout 超时时间
	// */
	// public RedisUtil(JedisPoolConfig config ,String ip, int prot ,int
	// timeout){
	// if (pool == null) {
	...
	public <T> T getObject(Object key) {
		...
		try {
			...
			jedis = pool.getResource();
			value = jedis.get(redisSerializer.serialize(key));
		} catch (Exception e) {
			pool.returnBrokenResource(jedis);
			e.printStackTrace();
		} finally {
	...
	public String setObject(Object key,Object value) {
		...
		try {
			...
			jedis = pool.getResource();
			value = jedis.set(redisSerializer.serialize(key),redisSerializer.serialize(value));
		} catch (Exception e) {
			pool.returnBrokenResource(jedis);
			e.printStackTrace();
		} finally {
